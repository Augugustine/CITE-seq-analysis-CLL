...
plotumap<-DimPlot(data_BxT, group.by = "final_annot", reduction = "umap", pt.size=0.1, label.size = 2.5) + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 15, hjust = 0.5), width = 3000, height = 2000, units = "px")
plotumap
ggsave(paste0(params$type,"_umap_BxT", params$patient, ".jpeg"), plotumap)
```

```{r}
Idents(data) <- "final_annot"
title=paste("UMAP focus on B cells and macrophages/monocytes, pre-processing type :", params$type, ", patient", params$patient)
highlighted_types <- c("Macrophage", "Monocyte", 
                       "B intermediate", "B memory", "B naive", "B")

data$highlight <- ifelse(data$final_annot %in% highlighted_types,
                         data$final_annot, "Other")
...
plotumap <- DimPlot(
  ...
  cols = mycols,
  pt.size = 0.1
) + ggtitle(str_wrap(title, width = 80)) +
  theme(plot.title = element_text(size = 15, hjust = 0.5))

plotumap
ggsave(paste0(params$type,"_umap_Bxmacro", params$patient, ".jpeg"), plotumap, width = 3000, height = 2000, units = "px")
```

```{r}
title=paste("UMAP focus on B cells and T cells, pre-processing type :", params$type, ", patient", params$patient)
highlighted_types <- c("CD4 T", "CD8 T", "T", 
                       "B intermediate", "B memory", "B naive", "B")
...
plotumap
ggsave(paste0(params$type,"_umap_BxT", params$patient, ".jpeg"), plotumap, width = 3000, height = 2000, units = "px")
```
