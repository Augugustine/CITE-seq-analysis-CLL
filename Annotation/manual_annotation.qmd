...
params: 
  patient: "P1"
  file: "CLL_auto_annotated_Seurat_LogNorm_P1.rds"
  type: "DDQC"
format: html
editor: visual
---
...
```{r}
cluster_order <- sort(unique(data$seurat_clusters))
annotation_order <- sort(unique(data$manual_annotation))
tab <- table(
  Cluster = factor(data$seurat_clusters, levels = cluster_order),
  CellType = factor(data$manual_annotation, levels = annotation_order)
)
tab_log <- log1p(tab)
