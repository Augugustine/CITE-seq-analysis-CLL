...
params: 
  patient: "P1"
  file: "/home/ablanc/work/NetBIO2/DDQC/P1/CLL_P1_DDQC.rds"
  type: "DDQC"
format: html
editor: visual
---
...
norm_counts <- LayerData(data, assay = assay_to_use, layer = 'data')
```
...
plotwnn <- DimPlot(data_annotated, group.by="SingleR", reduction = 'wnn.umap', label = TRUE, repel = TRUE, label.size = 4) + ggtitle(str_wrap(title, width = 100)) + theme(plot.title = element_text(size = 15, hjust = 0.5), legend.text = element_text(size = 12))
plotwnn
...
#| include: false
#InstallData("pbmcref")
data_annotated <- RunAzimuth(data_annotated, reference = "pbmcref", assay=assay_to_use)
```
...
```{r}
cluster_order <- sort(unique(data_annotated$seurat_clusters))
annotation_order <- sort(unique(data_annotated$SingleR))
tab <- table(
  Cluster = factor(data_annotated$seurat_clusters, levels = cluster_order),
  CellType = factor(data_annotated$SingleR, levels = annotation_order)
)
tab_log <- log1p(tab)
...
ggsave(paste0(params$type,"_heatmap_singleR_", params$patient, ".jpeg"), p2, width = 3000,
  height = 2000, units = "px")

annotation_order <- sort(unique(data_annotated$predicted.celltype.l1))
tab <- table(
  Cluster = factor(data_annotated$seurat_clusters, levels = cluster_order),
  CellType = factor(data_annotated$predicted.celltype.l1, levels = annotation_order)
)
tab_log <- log1p(tab)
...
ggsave(paste0(params$type,"_heatmap_Azimuth_", params$patient, ".jpeg"), p1, width = 3000,
  height = 2000, units = "px")
```
