...
```{r}
title=paste("UMAP annotated by Azimuth, pre-processing type :", params$type, ", patient", params$patient)
plotumap<-DimPlot(data_annotated, group.by = "predicted.celltype.l2", reduction = "umap", label = TRUE, repel = TRUE, label.size = 2.5) + ggtitle(str_wrap(title, width = 50)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
plotumap
ggsave(paste0(params$type,"_umap_Azimuth_l2_", params$patient, ".jpeg"), plotumap)
```
