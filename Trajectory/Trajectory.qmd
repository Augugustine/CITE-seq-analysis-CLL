...
cds <- learn_graph(cds)
cds <- order_cells(cds, root_cells = colnames(data)[data$timepoint == "D1"])

title=paste("Trajectory by Monocle3, patient", params$patient)
plot_traj<-plot_cells(cds,
           color_cells_by = "final_annot",
           label_groups_by_cluster=FALSE,
           label_leaves=FALSE,
           label_branch_points=FALSE) + ggtitle(title)
ggsave(paste0(params$type,"_trajectory_", params$patient, ".jpeg"), plot_traj, width = 5000,
  height = 2000, units = "px")
```
...
```{r}
title=paste("Pseudotime trajectory by Monocle3, patient", params$patient)
plot_traj<-plot_cells(cds, color_cells_by = "pseudotime") + ggtitle(title)
ggsave(paste0(params$type,"_trajectory_pseudotime_", params$patient, ".jpeg"), plot_traj, width = 5000,
  height = 2000, units = "px")
```
