...
```{r}
#############################################################
# Analyse de l'évolution des activités TF à travers les timepoints
# + calcul des deltas (variations) entre timepoints consécutifs
# et entre D1 et D14 pour l'ensemble des TF
#############################################################

library(dplyr)
library(tidyr)
library(ggplot2)
library(forcats)   # pour réordonner les facteurs dans le plot 2

## -----------------------------------------------------------
## 1. CHARGEMENT DES DONNEES
## -----------------------------------------------------------
# Remplace le chemin par le tien. La matrice doit avoir :
#   - une colonne "Timepoint" (valeurs: D1, D4, D8, D11, D14)
#   - une colonne d'ID cellule (optionnel, adapte le nom si besoin)
#   - une colonne par TF (254 colonnes numériques)
...
# Ordre logique des timepoints
tp_order <- c("D1", "D4", "D8", "D11", "D14")
df$Timepoint <- factor(df$timepoint, levels = tp_order)

# Liste des colonnes TF = tout sauf Timepoint (et un éventuel ID cellule)
# Adapte "CellID" si ta colonne d'identifiant cellule a un autre nom
non_tf_cols <- c("Timepoint", "CellID")
tf_cols <- setdiff(colnames(df), non_tf_cols)

## -----------------------------------------------------------
## 2. AGREGATION : moyenne d'activité par TF et par timepoint
##    (cellules indépendantes -> on moyenne, pas de tracking)
## -----------------------------------------------------------
mean_activity <- df %>%
  group_by(Timepoint) %>%
...
# Passage en format long : une ligne = (Timepoint, TF, activité moyenne)
mean_activity_long <- mean_activity %>%
  pivot_longer(cols = all_of(tf_cols), names_to = "TF", values_to = "mean_activity")

## -----------------------------------------------------------
## 3. CALCUL DES DELTAS CONSECUTIFS (D4-D1, D8-D4, D11-D8, D14-D11)
## -----------------------------------------------------------
...
## -----------------------------------------------------------
## 4. GRAPHIQUE 1 : évolution du delta pour des TF spécifiques
tf_of_interest <- c("CEBPA", "SPI1", "NANOG")   # <-- à adapter
...
p1 <- ggplot(plot_data_1, aes(x = transition, y = delta, color = TF, group = TF)) +
  ...
  geom_point(size = 2.5) +
  labs(
    title = "Variation de l'activité des TF entre timepoints consécutifs",
    x = "Transition entre timepoints",
    y = "Delta d'activité (Tn - Tn-1)",
    color = "TF"
  ) +
...
nanog_df$timepoint <- factor(
  nanog_df$timepoint,
  levels = c("D1", "D4", "D8", "D11", "D14")
)
ggplot(nanog_df, aes(x = timepoint, y = SPI1, fill = timepoint)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = 0.1, fill = "white") +
  theme_classic() +
  labs(
    title = "NANOG TF activity across timepoints",
    x = "Timepoint",
    y = "NANOG activity"
  )
...
ggplot(nanog_df, aes(x = timepoint, y = CEBPA, fill = timepoint)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = 0.1, fill = "white") +
  theme_classic() +
  labs(
