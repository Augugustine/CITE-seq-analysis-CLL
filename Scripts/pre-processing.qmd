...
```{r}
CLL_p1_merged <- AddMetaData(CLL_p1_merged, results_multiplet, col.names=multiplet)
CLL_p1_merged <- subset(CLL_p1_merged, multiplet == 'singlet')
```
...
CLL_p1_merged <- FindClusters(CLL_p1_merged, resolution = 0.5)
```
...
```{r}
saveRDS(CLL_p1_merged, file = "./pbmc_tutorial.rds")
```
