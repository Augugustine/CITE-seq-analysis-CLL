...
Memory optimization, remove the data unused
...
CLL_p1_merged <- FindClusters(CLL_p1_merged, resolution = 0.2)
```
...
```{r}
#| warning: false
CLL_p1_merged <- RunUMAP(CLL_p1_merged, dims = 1:10)
DimPlot(CLL_p1_merged, reduction = "umap")
```

Save the object after the pre-processing

```{r}
saveRDS(CLL_p1_merged, file = "./CLL_p1_merged.rds")
```
