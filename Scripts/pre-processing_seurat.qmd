...
```{r}
DimPlot(CLL_multiplet, group.by = "Doublet_Status", cols = c("Other" = "grey80", "Doublet" = "red")) + ggtitle(paste("Doublets position for", params$patient, "\nwith DoubletFinder (McGinnis, C. S. and al, 2019)"))
```
...
```{r}
DimPlot(CLL_qc, group.by = "qc_pass", cols = c("TRUE" = "grey80", "FALSE" = "red")) + ggtitle(paste("QC status of cells (red = failed) for", params$patient,"based on \npredefined thresholds for nFeature and mitochondrial \ngene percentage (Seurat pipeline)"))
```
...
DimPlot(CLL_merged_sctransform, reduction = "umap") + ggtitle(paste("UMAP (SCTransform-normalized, post-Seurat pipeline), for,", params$patient))
plotumap<-DimPlot(CLL_merged_sctransform, reduction = "umap")
ggsave("umap1.jpeg", plotumap)
```
...
DimPlot(CLL_merged_sctransform, reduction = "umap", group.by = "seurat_clusters", split.by = "timepoint") +
  ggtitle(paste("UMAP by timepoint (SCTransform-normalized, post-Seurat pipeline), for,", params$patient)) +
...
DimPlot(CLL_merged, reduction = "umap", group.by = "seurat_clusters", split.by = "timepoint") +
  ggtitle(paste("UMAP by timepoint (LogNormalize-normalized, post-Seurat pipeline), for,", params$patient)) +
  theme(plot.title = element_text(hjust = 0.5))
```
...
TSNEPlot(CLL_merged_sctransform) + ggtitle(paste("t-SNE (normalized by SCTransform, post-Seurat pipeline), for", params$patient))
```

```{r}
CLL_merged <- RunTSNE(CLL_merged, reduction.use = "pca", dims.use = 1:10)
TSNEPlot(CLL_merged) + ggtitle(paste("t-SNE (normalized by LogNormalize, post-Seurat pipeline), for", params$patient))
```
...
write.table(counts, file = "CLL_P1_seurat_SCTransform.tsv", sep = "\t", quote = FALSE, col.names = NA)
```

```{r}
counts <- GetAssayData(object = CLL_merged, assay = "RNA", slot = "counts")
counts <- as.matrix(counts)
write.table(counts, file = "CLL_P1_seurat_logNormalize.tsv", sep = "\t", quote = FALSE, col.names = NA)
```
...
plotwnn <- DimPlot(CLL_merged_sctransform, reduction = 'wnn.umap', label = TRUE, repel = TRUE, label.size = 2.5) + 
  NoLegend() + ggtitle(paste("wnnUMAP on RNA SCTranform normalization and post-Seurat pipeline, for", params$patient))
plotwnn
ggsave("wnnumpa_sct.jpeg", plotwnn)
...
plotwnn <- DimPlot(CLL_merged, reduction = 'wnn.umap', label = TRUE, repel = TRUE, label.size = 2.5) + 
  NoLegend() + ggtitle(paste("wnnUMAP with RNA LogNormalize and post-Seurat pipeline, for", params$patient))
plotwnn
ggsave("wnnumpa_log.jpeg", plotwnn)
