...
```{r}
DimPlot(CLL_multiplet, group.by = "Doublet_Status", cols = c("Other" = "grey80", "Doublet" = "red")) + ggtitle("Doublets position")
```
...
```{r}
DimPlot(CLL_qc, group.by = "qc_pass", cols = c("TRUE" = "grey80", "FALSE" = "red")) + ggtitle("QC status of cells (red = failed)")
```
...
DimPlot(CLL_merged_sctransform, reduction = "umap") + ggtitle("UMAP (normalized by SCTransform)")
...
DimPlot(CLL_merged_sctransform, reduction = "umap", group.by = "seurat_clusters", split.by = "timepoint") +
  ggtitle("UMAP by timepoint (normalized by SCTransform)") +
...
DimPlot(CLL_merged, reduction = "umap", group.by = "seurat_clusters", split.by = "timepoint") +
  ggtitle("UMAP by timepoint (normalized by LogNormalize)") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
TSNEPlot(CLL_merged_sctransform) + ggtitle("t-SNE (normalized by SCTransform)")
```

```{r}
CLL_merged <- RunTSNE(CLL_merged, reduction.use = "pca", dims.use = 1:10)
TSNEPlot(CLL_merged) + ggtitle("t-SNE (normalized by LogNormalize)")
```
...
plotwnn <- DimPlot(CLL_merged_sctransform, reduction = 'wnn.umap', label = TRUE, repel = TRUE, label.size = 2.5) + 
  NoLegend() + ggtitle("wnnUMAP on RNA SCTranform normalization")
plotwnn
...
plotwnn <- DimPlot(CLL_merged, reduction = 'wnn.umap', label = TRUE, repel = TRUE, label.size = 2.5) + 
  NoLegend() + ggtitle("wnnUMAP on RNA LogNormalize normalization")
plotwnn
