...
doublet
ggsave(paste("doublet_", params$patient, ".jpeg"), doublet)
```
...
kmeanplot
ggsave(paste("kmean_", params$patient, ".jpeg"), kmeanplot)
```
...
time
ggsave(paste("seurat_umap_timepoint_sct_", params$patient, ".jpeg"), time)
```
...
ggsave(paste("seurat_umap_log_", params$patient, ".jpeg"), plotumap)
```
...
time2<-DimPlot(CLL_merged, reduction = "umap", group.by = "seurat_clusters", split.by = "timepoint") +
  ggtitle(paste("UMAP by timepoint (LogNormalize-normalized, post-Seurat pipeline),", params$patient)) +
  theme(plot.title = element_text(hjust = 0.5))
time2
...
tsne
ggsave(paste("seurat_tsne_sct_", params$patient, ".jpeg"), tsne)
```
...
tsne1
ggsave(paste("seurat_tsne_log_", params$patient, ".jpeg"), tsne1)
```
...
plotwnn <- DimPlot(CLL_merged_sctransform, reduction = 'wnn.umap', label = TRUE, repel = TRUE, label.size = 2.5) + 
  NoLegend() + ggtitle(paste("wnnUMAP on RNA SCTranform normalization \nand post-Seurat pipeline,", params$patient))
plotwnn
ggsave(paste("seurat_wnnumpa_sct_", params$patient, ".jpeg"), plotwnn)
...
plotwnn
ggsave(paste("seurat_wnnumpa_log_", params$patient, ".jpeg"), plotwnn)
