...
title=paste("UMAP by timepoint (SCTransform-normalized, post-Seurat pipeline),", params$patient)
...
time
ggsave(paste0("seurat_umap_timepoint_sct_", params$patient, ".jpeg"), time)
```

```{r}
#| warning: false
CLL_merged <- RunUMAP(CLL_merged, dims = 1:10)
title=paste("UMAP (normalized by LogNormalize, post-Seurat pipeline),", params$patient)
...
ggsave(paste0("seurat_umap_log_", params$patient, ".jpeg"), plotumap)
```
...
title=paste("UMAP by timepoint (LogNormalize-normalized, post-Seurat pipeline),", params$patient)
...
time2
ggsave(paste0("seurat_umap_timepoint_log_", params$patient, ".jpeg"), time2)
```
...
title=paste("t-SNE (normalized by LogNormalize, post-Seurat pipeline),", params$patient)
tsne1<-TSNEPlot(CLL_merged) + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
tsne1
ggsave(paste0("seurat_tsne_log_", params$patient, ".jpeg"), tsne1)
```
...
counts <- as.matrix(counts)
namefile=paste0("CLL_", params$patient, "_seurat_logNormalize.tsv")
write.table(counts, file = namefile, sep = "\t", quote = FALSE, col.names = NA)
```
...
plotwnn <- DimPlot(CLL_merged_sctransform, reduction = 'wnn.umap', label = TRUE, repel = TRUE, label.size = 2.5) + NoLegend() + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
plotwnn
...
title=paste("wnnUMAP with RNA LogNormalize and post-Seurat pipeline,", params$patient)
plotwnn <- DimPlot(CLL_merged, reduction = 'wnn.umap', label = TRUE, repel = TRUE, label.size = 2.5) + NoLegend() + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
plotwnn
ggsave(paste0("seurat_wnnumpa_log_", params$patient, ".jpeg"), plotwnn)
...
```{r}
namepath=paste0("./CLL_", params$patient, "_seurat.rds")
