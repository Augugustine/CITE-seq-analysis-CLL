...
Scaling 
Remove of unwanted sources of variation
```{r}
...
# 1st exploring 
```{r}
pbmc <- RunPCA(CLL_p1_merged, features = VariableFeatures(object = CLL_p1_merged))
```
...
## Doublet removal
Important to do this step on normalized and scaled data. The tool DoubletFinder can't be applied on aggregated data. The samples (here the days) should be unmerged.
...
2- Finding the optimal arguments, PCs, pK, and nExp, (keep pN=0,25)  for the function DoubletFinder
```{r cars}
