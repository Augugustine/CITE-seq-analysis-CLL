...
title=paste("UMAP without QC and filtering,", params$patient)
...
ggsave(paste0("withoutQC_umap_", params$patient, ".jpeg"), plotumap)
```
...
title=paste("UMAP by timepoint without QC and filtering,", params$patient)
...
plot1
ggsave(paste0("withoutQC_umap_timepoint_", params$patient, ".jpeg"), plot1)
```
...
title=paste("t-SNE without QC and filtering,", params$patient)
plot1<-TSNEPlot(CLL_merged) + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
plot1
ggsave(paste0("withoutQC_tsne_", params$patient, ".jpeg"), plot1)
```
...
title=paste("wnnUMAP without QC and filtering,", params$patient)
...
plotwnn
ggsave(paste0("withoutQC_wnnumap_", params$patient, ".jpeg"), plotwnn)
