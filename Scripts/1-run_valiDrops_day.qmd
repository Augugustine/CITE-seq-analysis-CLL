...
This script applies the valiDrops tool to files from individual days, since running it on merged data (all days combined) is computationally too slow. The valiDrops method identifies and filters empty barcodes as well as dead cells, with empty barcodes being removed directly in this step. And the non coding genes are removed. The script processes one day’s dataset for a given patient, applies valiDrops, and then performs doublet removal using DoubletFinder. The output is a Seurat object stored in the corresponding patient/day folder, where metadata columns indicate whether cells are alive or dead, singlets or doublets, and with empty barcodes already removed. This object is subsequently used in the `pre-processing_valiDrops` script.
...
seurat_object <- subset(seurat_object, subset = qc.pass == "pass")
```
...
```{r}
filepaths <- file.path(params$directory, params$patient, paste0("run_count_J", params$day), "outs", "valiDrops.rds")
