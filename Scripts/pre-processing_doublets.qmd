...
plotumap<-DimPlot(CLL_merged, reduction = "umap")  + ggtitle(paste("UMAP without the doublets \nremoval,", params$patient))
...
time2<-DimPlot(CLL_merged, reduction = "umap", group.by = "seurat_clusters", split.by = "timepoint") +
  ggtitle(paste("UMAP by timepoint without the doublets \nremoval,", params$patient)) +
...
tsne1<-TSNEPlot(CLL_merged) + ggtitle(paste("t-SNE without the doublets \nremoval,", params$patient))
tsne1
...
plotwnn <- DimPlot(CLL_merged, reduction = 'wnn.umap', label = TRUE, repel = TRUE, label.size = 2.5) + 
  NoLegend() + ggtitle(paste("wnnUMAP without the doublets \nremoval,", params$patient))
plotwnn
