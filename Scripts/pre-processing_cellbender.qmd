...
params: 
  ...
  file: "cellbender_seurat_compatible.h5"
format: html
editor: visual
---
...
Before running this script, CellBender (via `cell_bender.py`) must be applied to the data, followed by conversion to Seurat-compatible format using `compatible_seurat_cellbender.sh`
...
doublet<-DimPlot(CLL_multiplet, group.by = "Doublet_Status", cols = c("Other" = "grey80", "Doublet" = "red")) + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
doublet
...
```{r}
title=paste("QC status of cells (red = failed) for", params$patient,"based on predefined thresholds for nFeature and mitochondrial gene percentage (Seurat pipeline and CellBender)")
qc<-DimPlot(CLL_qc, group.by = "qc_pass", cols = c("TRUE" = "grey80", "FALSE" = "red")) + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
qc
...
title=paste("UMAP (SCTransform-normalized, post-Seurat pipeline and CellBender),", params$patient)
DimPlot(CLL_merged_sctransform, reduction = "umap") + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
plotumap<-DimPlot(CLL_merged_sctransform, reduction = "umap") + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
...
title=paste("UMAP by timepoint (SCTransform-normalized, post-Seurat pipeline and CellBender),", params$patient)
time<-DimPlot(CLL_merged_sctransform, reduction = "umap", group.by = "seurat_clusters", split.by = "timepoint") + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
time
...
title=paste("UMAP (normalized by LogNormalize, post-Seurat pipeline and CellBender),", params$patient)
DimPlot(CLL_merged, reduction = "umap") + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
plotumap<-DimPlot(CLL_merged, reduction = "umap") + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
...
title=paste("UMAP by timepoint (LogNormalize-normalized, post-Seurat pipeline and CellBender),", params$patient)
time2<-DimPlot(CLL_merged, reduction = "umap", group.by = "seurat_clusters", split.by = "timepoint") + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
time2
...
title=paste("t-SNE (normalized by SCTransform, \npost-Seurat pipeline and CellBender),", params$patient)
tsne<-TSNEPlot(CLL_merged_sctransform) + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
tsne
...
title=paste("t-SNE (normalized by LogNormalize, \npost-Seurat pipeline and CellBender),", params$patient)
tsne1<-TSNEPlot(CLL_merged) + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
tsne1
...
title=paste("wnnUMAP with RNA LogNormalize \nand post-Seurat pipeline and CellBender,", params$patient)
