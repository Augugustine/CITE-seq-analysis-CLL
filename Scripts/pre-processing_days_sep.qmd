...
plotumap<-DimPlot(CLL_merged, reduction = "umap") + ggtitle(paste("UMAP with QC applied separately \nfor each day,", params$patient))
...
plot1<-DimPlot(CLL_merged, reduction = "umap", group.by = "seurat_clusters", split.by = "timepoint") +
  ggtitle(paste("UMAP by timepoint with QC applied separately \nfor each day,", params$patient)) +
...
plot1<-TSNEPlot(CLL_merged) + ggtitle(paste("t-SNE with QC applied separately \nfor each day,", params$patient))
plot1
...
plotwnn <- DimPlot(CLL_merged, reduction = 'wnn.umap', label = TRUE, repel = TRUE, label.size = 2.5) + 
  NoLegend() + ggtitle(paste("wnnUMAP with QC applied separately \nfor each day,", params$patient))
plotwnn
