...
```{r}
title=paste("QC status of cells (red = failed) using valiDrops tool (filtering the empty barcodes and dead cells,", params$patient)
plot1<-DimPlot(CLL_qc, group.by = "qc_final", cols = c("TRUE" = "grey80", "FALSE" = "red")) + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
plot1
...
```{r}
title=paste("Doublets position with DoubletFinder (McGinnis, C. S. and al, 2019)", params$patient)
...
title=paste("UMAP by timepoint after QC and filtering by valiDrops,", params$patient)
plot<-DimPlot(CLL_merged, reduction = "umap", group.by = "seurat_clusters", split.by = "timepoint") + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
plot
ggsave(paste0("valiDrops_umap_timepoint_", params$patient, ".jpeg"), plotumap)
```
