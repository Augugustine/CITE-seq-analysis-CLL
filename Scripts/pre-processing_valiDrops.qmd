...
```{r}
plot1<-DimPlot(CLL_qc, group.by = "qc_final", cols = c("TRUE" = "grey80", "FALSE" = "red")) + ggtitle(paste("QC status of cells (red = failed) using \nvaliDrops tool (filtering the empty barcodes \nand dead cells,", params$patient))
plot1
...
plotumap<-DimPlot(CLL_merged, reduction = "umap")  + ggtitle(paste("UMAP after QC and filtering \nby valiDrops,", params$patient))
...
plot1<-TSNEPlot(CLL_merged) + ggtitle(paste("t-SNE after QC and filtering \nby valiDrops,", params$patient))
plot1
