...
Steps of this pre-processing, Seurat, DoubletFinder and miQC (all days merged)

-   Downloading the data and creating one Seurat object per day. Using filtered data without empty drops and remove technical artifacts (from CellRanger 10X Genomics or CellBender)
...
This step in done on the data days merged (same threshold for the 5 time points). miQC is a tool for filtering low-quality cells in single-cell RNA-seq data. It uses a probabilistic model based on two metrics: the percentage of mitochondrial reads and the number of detected genes. Instead of using fixed thresholds, miQC learns the distribution of these values in your data and estimates the probability that each cell is low quality. You can then filter out cells with high probabilities of being compromised (by default posterior \> 0.75) (Hippen et al., 2021).
...
Applying the miQC tool on the data transformed in SingleCellExperiment format

```{r}
#| warning: false
sce <- as.SingleCellExperiment(CLL_merged)
...
```{r}
miqc_metadata <- as.data.frame(colData(sce_miqc)[, c("posterior", "discard")])
colnames(miqc_metadata) <- c("miqc_posterior", "miqc_pass")

CLL_merged <- AddMetaData(CLL_merged, metadata = miqc_metadata)
```
...
```{r}
#| warning: false
ggplot(CLL_merged@meta.data, aes(x = nFeature_RNA, y = percent.mt, color = miqc_posterior)) +
  geom_point(size = 1, alpha = 0.6) +
  scale_color_gradient(low = "blue", high = "red") +
  theme_minimal() +
  labs(title = "miQC Posterior Probabilities",
...
```{r}
ggplot(CLL_merged@meta.data, aes(x = nFeature_RNA, y = percent.mt, color = miqc_pass)) +
  geom_point(alpha = 0.5) +
  scale_color_manual(values = c("TRUE" = "green", "FALSE" = "grey")) +
  theme_minimal() +
  labs(title = "Cells kept (green) vs discarded (grey) by miQC",
       x = "Number of detected genes",
       y = "Percent mitochondrial reads",
       color = "miQC pass")
```
...
```{r}
CLL_merged <- subset(CLL_merged, subset = miqc_pass)
```
