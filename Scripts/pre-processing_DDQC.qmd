...
DimPlot(CLL_multiplet, group.by = "Doublet_Status", cols = c("Other" = "grey80", "Doublet" = "red")) + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
```
...
CLL_merged[["percent.rb"]] <- PercentageFeatureSet(CLL_merged, pattern = "^RPS|^RPL")
```

```{r}
ddqc_results<- ddqc.metrics(CLL_merged, res= 0.1, do.ribo = TRUE)
```

Metadata extraction and adding to the Seurat Object

```{r}
ddqc_metadata <- as.data.frame(ddqc_results[, c("passed.qc"), drop = FALSE])
...
```{r}
title=paste("QC status of cells (red = failed) for", params$patient,"using the DDQC tool (adaptive threshold based on the median absolute deviation on four QC metrics, gene and UMI complexity, fraction of reads mapping to mitochondrial and ribosomal genes)")
plot1<-DimPlot(CLL_qc, group.by = "passed.qc", cols = c("TRUE" = "grey80", "FALSE" = "red")) + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
plot1
ggsave(paste0("DDQC_qc_", params$patient, ".jpeg"), plot1)
```
...
plot2 + ggtitle(paste("Top variable features with the 10 most highly variable genes highlighted, for", params$patient))
```
...
title=paste("UMAP with QC using the DDQC tool,", params$patient)
...
ggsave(paste0("DDQC_umap_", params$patient, ".jpeg"), plotumap)
```
...
title=paste("UMAP by timepoint with QC using the DDQC tool,", params$patient)
plot1<-DimPlot(CLL_merged, reduction = "umap", group.by = "seurat_clusters", split.by = "timepoint") + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
plot1
ggsave(paste0("DDQC_umap_timepoint_", params$patient, ".jpeg"), plot1)
```
...
title=paste("t-SNE with QC using the DDQC tool,", params$patient)
plot1<-TSNEPlot(CLL_merged) + ggtitle(str_wrap(title, width = 80)) + theme(plot.title = element_text(size = 10, hjust = 0.5))
plot1
ggsave(paste0("DDQC_tsne_", params$patient, ".jpeg"), plot1)
```
...
namefile=paste0("CLL_", params$patient, "_DDQC.tsv")
...
title=paste("wnnUMAP with QC using the DDQC tool,", params$patient)
...
plotwnn
ggsave(paste0("DDQC_wnnumap_", params$patient, ".jpeg"), plotwnn)
...
```{r}
namepath=paste0("./CLL_", params$patient, "_DDQC.rds")
