...
```{r}
ddqc_metadata <- as.data.frame(ddqc_results[, c("passed.qc")])
...
```{r}
plot<-DimPlot(CLL_qc, group.by = "passed.qc", cols = c("TRUE" = "grey80", "FALSE" = "red")) + ggtitle(paste("QC status of cells (red = failed) for", params$patient,"using \nthe DDQC tool (adaptive threshold based on the median absolute\n deviation on four QC metrics, gene and UMI complexity, fraction \nof reads mapping to mitochondrial and ribosomal genes)"))
plot
ggsave(paste("DDQC_qc_", params$patient, ".jpeg"), plot)
```
...
DimHeatmap(CLL_merged, dims = 1, cells = 500, balanced = TRUE) + ggtitle(paste("Heatmap after the QC using the DDQC tool, for", params$patient))
```
...
ggsave(paste("DDQC_umap_", params$patient, ".jpeg"), plot)
```
...
plot
ggsave(paste("DDQC_umap_timepoint_", params$patient, ".jpeg"), plot)
```
...
plot<-TSNEPlot(CLL_merged) + ggtitle(paste("t-SNE with QC using the DDQC tool,", params$patient))
plot
ggsave(paste("DDQC_tsne_", params$patient, ".jpeg"), plot)
```
...
plotwnn
ggsave(paste("DDQC_wnnumap_", params$patient, ".jpeg"), plotwnn)
