...
```{r}
plot1<-DimPlot(CLL_qc, group.by = "passed.qc", cols = c("TRUE" = "grey80", "FALSE" = "red")) + ggtitle(paste("QC status of cells (red = failed) for", params$patient,"using \nthe DDQC tool (adaptive threshold based on the median absolute\n deviation on four QC metrics, gene and UMI complexity, fraction \nof reads mapping to mitochondrial and ribosomal genes)"))
plot1
...
plotumap<-DimPlot(CLL_merged, reduction = "umap") + ggtitle(paste("UMAP with QC using the DDQC tool,", params$patient))
...
plot1<-DimPlot(CLL_merged, reduction = "umap", group.by = "seurat_clusters", split.by = "timepoint") +
  ggtitle(paste("UMAP by timepoint with QC using the DDQC tool,", params$patient)) +
...
plot1<-TSNEPlot(CLL_merged) + ggtitle(paste("t-SNE with QC using the DDQC tool,", params$patient))
plot1
...
plotwnn <- DimPlot(CLL_merged, reduction = 'wnn.umap', label = TRUE, repel = TRUE, label.size = 2.5) + 
  NoLegend() + ggtitle(paste("wnnUMAP with QC using the DDQC tool,", params$patient))
plotwnn
